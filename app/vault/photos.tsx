...
export default function PhotosTab() {
    ...
    if (Platform.OS === 'android') {
      ...
      if (!hasAccess) {
        Alert.alert(
          ...
          'Yuklangan rasmlar galereyadan avtomatik va to\'liq o\'chishi uchun "Barcha fayllarga ruxsat" kerak. Ruxsat berilsinmi?',
          [
            {
              text: 'Keyinroq',
              style: 'cancel',
  ...
  const handleBatchExport = useCallback(async () => {
    let success = 0;
    for (const id of selectedIds) {
      const ok = await exportFile('photos', id);
      if (ok) success++;
    }
    Alert.alert('Exported', `${success} photo${success > 1 ? 's' : ''} restored to gallery.`);
    cancelSelect();
  ...
  const handleExport = useCallback(async (file: VaultFile) => {
    ...
    Alert.alert(
      ...
      success ? 'Photo restored to your gallery.' : 'Failed to export the photo.'
    );
  }, []);
